"""Notebook-friendly audio augmentation for hum/whistle classification.

This mirrors `augment.py` but is organized into sections for notebook cells.
"""
...
# ---------------------------------------------------------------------
# 7) Example "notebook flow" (copy into separate cells if desired)
# ---------------------------------------------------------------------
# cfg = AugmentConfig()
# rng = np.random.default_rng(42)
# sample_path = next(DATASET_DIR.glob("*.wav"))
# y = load_audio(sample_path, cfg)
# variants = augment_sample(y, cfg, rng)
# save_augmented(sample_path, variants, DEFAULT_OUTPUT_DIR, cfg)
